...
/* harmony import */ var _fullcalendar_timegrid__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @fullcalendar/timegrid */ "./node_modules/@fullcalendar/timegrid/index.js");
/**
...
function buildFilterButtons(filterButtons, state, getCalendar, opts) {
  ...
  var urlStateEnabled = !(opts && opts.urlState === false);
  ...
  filterButtons.forEach(function (btn) {
    ...
    buttonEl.addEventListener('click', function () {
      ...
      if (urlStateEnabled) {
        writeUrlState({
          filter: serializeFilterForUrl(state.filter)
        });
      }
...
function init(elementId, config) {
  ...
  // config otherwise.
  var urlStateEnabled = config.urlState !== false;
  var urlState = urlStateEnabled ? readUrlState() : {
    date: null,
    view: null,
    filter: null
  };
  ...
  var initialFilter = config.filter || null;
  if (urlState.filter) {
    ...
    initialFilter = config.filterMulti ? urlState.filter.split(',').filter(Boolean) : urlState.filter;
  }
  var state = {
  ...
  var eventsSource = config.events;
  ...
  var filterButtonNodes = {};
  if (Array.isArray(config.filterButtons) && config.filterButtons.length > 0) {
    var built = buildFilterButtons(config.filterButtons, state, getCalendar, {
  ...
  var options = Object.assign({
    ...
    // hasn't set it.
    firstDay: window.snipeit && window.snipeit.settings && typeof window.snipeit.settings.first_day_of_week === 'number' ? window.snipeit.settings.first_day_of_week : 0,
    ...
    datesSet: function datesSet(arg) {
  ...
  options.plugins = [_fullcalendar_daygrid__WEBPACK_IMPORTED_MODULE_0__["default"], _fullcalendar_timegrid__WEBPACK_IMPORTED_MODULE_1__["default"], _fullcalendar_list__WEBPACK_IMPORTED_MODULE_2__["default"], _fullcalendar_interaction__WEBPACK_IMPORTED_MODULE_3__["default"]];
  ...
  calendarInstance = new _fullcalendar_core__WEBPACK_IMPORTED_MODULE_4__.Calendar(el, options);
  ...
  if (filterContainer) {
    var initialActive = state.filter instanceof Set ? Array.from(state.filter) : state.filter ? [state.filter] : [];
    initialActive.forEach(function (name) {
      var btn = filterButtonNodes[name];
      if (btn) {
...
/******/ })()
;
//# sourceMappingURL=snipeit-calendar.js.map