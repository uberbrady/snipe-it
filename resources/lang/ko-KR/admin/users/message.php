<?php

return [

    'accepted' => '이 항목을 성공적으로 수락했습니다.',
    'declined' => '이 항목을 성공적으로 거절했습니다.',
    'accept_signature_write_failed' => 'Your acceptance signature could not be saved to storage. Acceptance was not recorded. Please contact your administrator.',
    'accept_pdf_write_failed' => 'The acceptance PDF could not be saved to storage. Acceptance was not recorded. Please contact your administrator.',
    'bulk_manager_warn' => '사용자가 성공적으로 갱신되었지만, 선택한 관리자가 편집할 사용자 목록에도 있었고, 사용자가 자신의 관리자가 아니 어서 관리자 항목이 저장되지 않았습니다. 관리자를 제외한 사용자를 다시 선택하십시오.',
    'user_exists' => '사용자가 이미 존재합니다!',
    'cannot_delete' => '사용자가 존재하지 않거나 삭제할 권한이 없습니다.',
    'user_not_found' => '사용자가 존재하지 않거나 조회 권한이 없습니다.',
    'user_login_required' => '로그인 항목을 입력해 주세요.',
    'user_has_no_assets_assigned' => '현재 사용자에게 할당된 자산이 없습니다.',
    'nothing_currently_assigned' => 'Nothing currently assigned.',
    'user_password_required' => '비밀번호를 입력해 주세요.',
    'insufficient_permissions' => '승인 불충분.',
    'auth_fields_denied' => 'You do not have permission to modify credential or activation fields on this user. Requested fields not updated: :fields',
    'user_deleted_warning' => '이 사용자는 삭제되었습니다. 그것들을 수정하려면 이 사용자를 복원하던가 새 자산들을 생성하세요.',
    'ldap_not_configured' => '이 설치의 LDAP 통합이 구성이 되지 않았습니다.',
    'password_resets_sent' => '활성화되어 있고 유효한 이메일 주소를 가진 선택된 사용자에게 비밀번호 재설정 링크가 전송되었습니다.',
    'not_activated' => '이 사용자는 로그인할 수 없으므로 이메일을 통해 자산을 수락할 수 없습니다.',
    'password_reset_sent' => ':email 로 패스워드 재설정 링크가 발송되었습니다.',
    'user_has_no_email' => '이 사용자의 프로필에 이메일 주소가 없습니다.',
    'log_record_not_found' => '이 사용자에 대한 일치하는 로그 기록을 찾을 수 없습니다.',

    'impersonate' => [
        'started' => '현재 :name(으)로 로그인되었습니다.',
        'stopped' => '본인 계정으로 돌아왔습니다.',
        'cannot_impersonate_self' => '자기 자신으로 로그인할 수 없습니다.',
        'cannot_impersonate_superuser' => '다른 슈퍼유저로 로그인할 수 없습니다.',
        'target_not_active' => '해당 사용자는 비활성화되었거나 삭제되어 로그인할 수 없습니다.',
        'impersonator_missing' => '이 대리 세션의 원래 계정이 더 이상 존재하지 않습니다. 다시 로그인해 주세요.',
    ],

    'success' => [
        'create' => '사용자가 생성되었습니다.',
        'update' => '사용자가 갱신 되었습니다.',
        'update_bulk' => '사용자가 갱신 되었습니다.',
        'delete' => '사용자가 삭제 되었습니다.',
        'ban' => '사용자가 금지 처리 되었습니다.',
        'unban' => '사용자의 금지 처리가 해제 되었습니다.',
        'suspend' => '사용자를 대기 시켰습니다.',
        'unsuspend' => '사용자의 대기를 해제하였습니다.',
        'restored' => '사용자를 복원하였습니다.',
        'import' => '사용자를 내보냈습니다.',
        'acceptance_reminder_sent' => ':count개의 대기 중인 항목에 대한 수락 알림을 보냈습니다.|:count개의 대기 중인 항목에 대한 수락 알림을 보냈습니다.',
    ],

    'error' => [
        'create' => '사용자를 생성하는 중 문제가 발생했습니다. 다시 시도해 주세요.',
        'update' => '사용자를 갱신하는 중 오류가 발생했습니다. 다시 시도해 주세요.',
        'delete' => '사용자를 삭제하는 중 문제가 발생했습니다. 다시 시도해 주세요.',
        'delete_has_assets' => '이 사용자에게 품목이 할당되었고 삭제할 수 없습니다.',
        'delete_has_assets_var' => '이 사용자에게 아직 할당된 자산이 있습니다. 먼저 반입하세요.|이 사용자에게 아직 :count개의 자산이 할당되어 있습니다. 먼저 자산을 반입하세요.',
        'delete_has_licenses_var' => '이 사용자에게 아직 할당된 라이선스 좌석이 있습니다. 먼저 반입하세요.|이 사용자에게 아직 :count개의 라이선스 좌석이 할당되어 있습니다. 먼저 반입하세요.',
        'delete_has_accessories_var' => '이 사용자에게 아직 할당된 액세서리가 있습니다. 먼저 반입하세요.|이 사용자에게 아직 :count개의 액세서리가 할당되어 있습니다. 먼저 자산을 반입하세요.',
        'delete_has_locations_var' => '이 사용자는 아직 위치를 관리하고 있습니다. 먼저 다른 관리자를 선택하세요.|이 사용자는 아직 :count개의 위치를 관리하고 있습니다. 먼저 다른 관리자를 선택하세요.',
        'delete_has_users_var' => '이 사용자는 아직 다른 사용자를 관리하고 있습니다. 먼저 해당 사용자의 다른 관리자를 선택하세요.|이 사용자는 아직 :count명의 사용자를 관리하고 있습니다. 먼저 해당 사용자들의 다른 관리자를 선택하세요.',
        'unsuspend' => '사용자의 대기 해제 중 문제가 발생했습니다. 다시 시도하세요.',
        'import' => '사용자를 내보내기 할 때 문제가 발생했습니다. 다시 시도하세요.',
        'asset_already_accepted' => '이 항목은 이미 수락되었습니다.',
        'accept_or_decline' => '이 자산을 승인 하거나 거부 하셔야 합니다.',
        'cannot_delete_yourself' => '본인을 삭제하면 안타까울 것 같습니다. 다시 한 번 생각해 주세요.',
        'incorrect_user_accepted' => '승인하려는 자산이 체크아웃되지 않았습니다.',
        'ldap_could_not_connect' => 'LDAP 서버에 접속 할 수 없습니다. LDAP 설정 파일의 LDAP 서버 구성을 확인해 보세요.<br>LDAP 서버 오류:',
        'ldap_could_not_bind' => 'LDAP 서버와 동기화 할 수 없습니다. LDAP 설정 파일의 LDAP 서버 구성을 확인해 보세요.<br>LDAP 서버 오류: ',
        'ldap_could_not_search' => 'LDAP 서버를 찾을 수 없습니다. LDAP 설정 파일의 LDAP 서버 구성을 확인해 보세요.<br>LDAP 서버 오류:',
        'ldap_could_not_get_entries' => 'LDAP 서버 목록을 가져올 수 없습니다. LDAP 설정 파일의 LDAP 서버 구성을 확인해 보세요.<br>LDAP 서버 오류:',
        'password_ldap' => '이 계정의 비밀번호는 LDAP/Active 디렉토리에 의해 관리됩니다. 비밀번호를 변경하려면 IT 부서에 문의하세요. ',
        'multi_company_items_assigned' => '이 사용자에게 다른 회사에 속한 항목이 할당되어 있습니다. 반입하거나 회사를 편집하세요.',
        'no_pending_acceptances' => '이 사용자는 알림을 보낼 대기 중인 수락 건이 없습니다.',
        'company_not_permitted' => 'One or more requested company assignments are outside your permitted company scope.',
    ],

    'deletefile' => [
        'error' => '파일이 삭제되지 않았습니다. 다시 시도해 주세요.',
        'success' => '파일이 삭제되었습니다.',
    ],

    'upload' => [
        'error' => '파일(들) 이 업로드 되지 않았습니다. 다시 시도해 주세요.',
        'success' => '파일(들) 이 업로드 되었습니다.',
        'nofiles' => '업로드 할 파일을 선택해 주세요',
        'invalidfiles' => '하나 이상의 파일이 너무 크거나 허용되지 않는 형식입니다. 허용되는 형식은 png, gif, jpg, doc, docx, pdf, txt 입니다.',
    ],

    'inventorynotification' => [
        'error' => '해당 유저는 이메일이 설정되어 있지 않습니다.',
        'success' => '사용자에게 현재 재고에 대한 알림이 전송되었습니다.',
    ],
];
