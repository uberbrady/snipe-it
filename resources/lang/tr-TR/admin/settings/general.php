...
return [
    ...
    'backups_restore_warning' => 'Geri yükleme butonunu <small><span class="btn btn-xs btn-warning"><i class="text-white fa-solid fa-trash-arrow-up" aria-hidden="true"></i></span></small> kullanarak önceki bir yedekten geri yükleme yapın. (Bu işlem şu an için S3 dosya depolama ile çalışmamaktadır.)<br><br><strong>Tüm :app_name veritabanınız ve yüklenen tüm dosyalar</strong>, yedek dosyasındakilerle tamamen değiştirilecektir  ',
    ...
    'api_request_filters' => 'Talep Filtreleme',
    'block_api_user_agents_text' => 'Kullanıcı Aracısına (User-Agent) göre API isteklerini engelle',
    'block_api_user_agents_help' => 'Etkinleştirildiğinde, Kullanıcı Aracısı aşağıdaki kalıplardan herhangi biriyle eşleşen API istekleri reddedilir.',
    'blocked_api_user_agents_text' => 'Engellenen API Kullanıcı Aracısı kalıpları',
    'blocked_api_user_agents_help' => 'Her satıra bir kalıp yazın. Her kalıp, isteğin Kullanıcı Aracısı (User-Agent) başlangıcına göre büyük/küçük harf duyarlılığı olmadan eşleştirilir. Örneğin "curl/", "curl/8.5.0" ile eşleşir, ancak Kullanıcı Aracısı içinde "curl/" ifadesi daha sonra geçen bir metinle eşleşmez. Metin alanı, yaygın betik tabanlı istemcilerle (curl, Postman, python-requests, vb.) önceden doldurulmuştur. İhtiyaca göre satır ekleyin, kaldırın veya temizleyin.',
    'block_blank_api_user_agents_text' => 'Ayrıca, Kullanıcı Aracısı boş olan API isteklerini de reddet.',
    'block_blank_api_user_agents_help' => 'Ayrıca, Kullanıcı Aracısı başlığı boş veya eksik olarak gelen istekleri de reddet. <strong>SCIM uç noktaları bu ayarı yok sayar ve boş Kullanıcı Aracısını her zaman kabul eder, çünkü Microsoft Entra ID SCIM sağlama işlemi boş bir Kullanıcı Aracısı gönderir.</strong> Boş Kullanıcı Aracısı gönderen başka meşru entegrasyonlarınız varsa bu seçeneği kapalı bırakın.',
    'blocked_api_user_agent_rejected' => 'Bu API uç noktası, isteği Kullanıcı Aracısına göre reddetti.',
    ...
    'null_company_is_floater_text' => 'Şirket eşleştirmesi olmayan varlıkları ve kullanıcıları serbest olarak değerlendir',
    'null_company_is_floater_help_text' => 'Devre dışı bırakıldığında, şirket eşleştirmesi olmayan varlıklar yalnızca varlığın da bir şirketi yoksa zimmetlenebilir - null değeri kendi başına sahte bir şirket olarak kabul edilir. Etkinleştirildiğinde ise, herhangi bir şirkete ait varlıklar, şirket ataması olmayan kişilere/yerlere zimmetlenebilir.',
...
<a href=":webhook_link" target="_new" rel="noopener">gelen bir webhook oluştur</a>:app hesabınızda. Tıkla<strong>Test :app Entegrasyonu</strong>Kaydetmeden önce ayarlarınızın doğru olduğunu onaylamak için düğmesine basın. ',
    ...
    'remote_user_legend_warning' => 'Dikkatli kullanın! Bu bölümün hatalı yapılandırılması, sizin ve kullanıcılarınızın sistem dışında kalmasına veya daha da kötüsü, kullanıcıların otomatik olarak belirli bir kullanıcı adıyla giriş yapmasına neden olabilir.',
    'login_remote_user_header_name_help' => 'Web sunucunuz, kullanıcı adını içeren özel bir başlık (header) gönderecek şekilde yapılandırılmışsa, bu başlığı burada belirtebilirsiniz. Örneğin, web sunucunuz kullanıcı adıyla birlikte <code>X-Remote-User</code> adlı bir başlık gönderiyorsa, bu alana <code>HTTP_X_REMOTE_USER</code> yazmalısınız. Bu alan boş bırakılırsa, sistem varsayılan olarak <code>REMOTE_USER</code> başlığını kullanacaktır.

<strong>Bu ayarı yanlış yapılandırmanız, hem sizi hem de diğer tüm kullanıcıları sistemden kilitleyebilir VEYA herkesin sisteme süper yönetici (superadmin) olarak otomatik şekilde giriş yapmasına neden olabilir!</strong>',
