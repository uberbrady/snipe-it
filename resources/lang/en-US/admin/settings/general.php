...
return [
    ...
    'ldap_wizard' => [
        ...
        'step_mapping_help' => 'Specify which LDAP attributes hold each piece of user information. <strong>Always use the lower-case form of the attribute name.</strong> If your LDAP or AD shows the field as <code>displayName</code>, <code>givenName</code>, or <code>sAMAccountName</code>, enter <code>displayname</code>, <code>givenname</code>, or <code>samaccountname</code> here. Camel-case values fail to match on most servers and are the most common cause of "no fields sync" reports. Username and First Name are required. You can leave others blank if your directory does not carry that data or if you do not want to sync it.',
        'step_sync_help' => 'Final step: enable LDAP and pick the defaults to apply to newly-synced users. When the enable toggle is on and saved, LDAP is live, users can sign in with their directory credentials and admin sync jobs will start pulling accounts.',
        'sync' => [
        ...
        'done' => [
            ...
            'intro' => 'Your directory is connected and users can now sign in with their LDAP credentials. Consider setting up a scheduled sync so new and updated users are pulled from your directory automatically.',
            'sync_heading' => 'Keep users in sync with your directory',
            'sync_intro' => 'User syncing is a separate step from authentication. Log-in works right now, but existing accounts are only refreshed when a sync runs. Pick the option that matches your host:',
            'sync_cron_title' => 'Linux, macOS, and other Unix-like hosts',
            'sync_cron_body' => 'Add this entry to the crontab of the web-server user (usually <code>www-data</code>). It runs the LDAP sync every hour:',
            'sync_windows_title' => 'Windows',
            'sync_windows_body' => 'Create a Scheduled Task that runs the command below every hour, with the working directory set to your installation path:',
            'sync_manual_title' => 'Run a one-off sync from the command line',
            'sync_manual_body' => 'Useful for the first sync or when troubleshooting. Run from your installation directory:',
