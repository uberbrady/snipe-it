<?php

return [
    'about_assets_title' => '關於資產',
    'about_assets_text' => '資產是按序號或資產標籤追蹤的物品。 他們往往是高價值並標示為重要的物品。',
    'archived' => '已封存',
    'asset' => '資產',
    'bulk_checkout' => '批次借出',
    'bulk_checkin' => '批次繳回',
    'bulk_audit' => '批次稽核',
    'bulk_audit_location_hidden_mixed_companies' => 'The audit location field is hidden because the selected assets belong to multiple companies. With location scoping enabled, one shared audit location can\'t apply to assets from different companies. Audit each company\'s assets in a separate batch to set an audit location.',
    'checkin' => '資產繳回',
    'checkin_assets' => '資產繳回',
    'checkout' => '借出資產',
    'clear' => '清除',
    'clone' => '複製資產',
    'deployable' => '可部署',
    'deleted' => '此資產已被刪除.',
    'delete_confirm' => '您確定要刪除此資產嗎？',
    'edit' => '編輯資產',
    'model_deleted' => '此資產模板已被刪除. 你必須先還原資產模板才可還原資產.',
    'model_invalid' => '此資產的型號無效。',
    'model_invalid_fix' => '在嘗試借出、繳回或稽核此資產之前，必須先更新為有效的資產型號。',
    'requestable' => '可申領',
    'requested' => '已申領',
    'not_requestable' => '不可申請',
    'requestable_status_warning' => '請不要更改可申請狀態',
    'require_serial' => '要求序號',
    'require_serial_help' => '建立此型號的新資產時，序號為必填欄位。',
    'restore' => '還原資產',
    'pending' => '待處理',
    'undeployable' => '不可部署',
    'undeployable_tooltip' => '此資產的狀態標籤為不可部署，因此目前無法借出。',
    'view' => '檢視資產',
    'csv_error' => '你的 CSV 檔案有錯誤',
    'import_text' => '<p>上傳包含資產歷史記錄的 CSV 檔。資產和使用者必須已存在於系統中，否則將被跳過。歷史記錄匯入的資產比對依據為資產標籤。系統將根據您提供的使用者姓名和以下選取的條件嘗試找到符合的使用者。若未選取任何條件，將嘗試依照您在 <code>管理員 &gt; 一般設定</code> 中設定的使用者名稱格式進行比對。</p><p>CSV 中的欄位必須符合以下標題：<strong>資產標籤、名稱、借出日期、繳回日期</strong>。其他額外欄位將被忽略。</p><p>繳回日期：空白或未來的繳回日期將把物品借出給關聯使用者。省略繳回日期欄位將以今天日期建立繳回日期。</p>',
    'csv_import_match_f-l' => '嘗試以 <strong>名字.姓氏</strong>（<code>jane.smith</code>）格式比對使用者',
    'csv_import_match_initial_last' => '嘗試以 <strong>名字縮寫+姓氏</strong>（<code>jsmith</code>）格式比對使用者',
    'csv_import_match_first' => '嘗試以 <strong>名字</strong>（<code>jane</code>）格式比對使用者',
    'csv_import_match_email' => '嘗試以 <strong>電子郵件</strong> 作為使用者名稱比對使用者',
    'csv_import_match_username' => '嘗試以 <strong>使用者名稱</strong> 比對使用者',
    'error_messages' => '錯誤訊息:',
    'success_messages' => '成功訊息:',
    'alert_details' => '請看下面的詳細資料.',
    'custom_export' => '自定義匯出',
    'mfg_warranty_lookup' => ':manufacturer 保固狀態查詢',
    'user_department' => '使用者部門',
];
